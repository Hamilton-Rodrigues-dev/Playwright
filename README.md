...
## 📚 11. Exemplo de uso completo(possui algumas divergencias)
...
test("exemplo completo - formulário de contato", async ({ page }) => {
  ...
  const continenteSelecionado = await page
    .locator("select#continents")
    .inputValue();
...
test("extrair dados de tabela", async ({ page }) => {
  ...
  const primeiraLinha = await page
    .locator("table#customers tr")
    .nth(1)
    .textContent();
  console.log("Primeira linha:", primeiraLinha);

  // Pegar todos os dados de uma coluna
  const empresas = await page
    .locator("table#customers td:nth-child(1)")
    .allTextContents();
...
test("buscar no Google e pegar resultados", async ({ page }) => {
  ...
  const links = await page
    .locator("a h3")
    .evaluateAll((elements) => elements.map((el) => el.closest("a").href));
